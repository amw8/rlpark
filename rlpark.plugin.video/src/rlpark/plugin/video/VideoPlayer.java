...
public class VideoPlayer implements ZephyrRunnable {
   ...
   * 
   */
  private ImageListener mScreen = null;
    ...
    while (container.readNextPacket(packet) >= 0) {
      ...
      if (packet.getStreamIndex() == videoStreamId) {
        ...
        if (picture.isComplete()) {
          ...
          // if there is no audio stream; go ahead and hold up the main thread.
          // We'll end
          ...
          if (imageConverter == null)
            imageConverter = ConverterFactory.createConverter(ConverterFactory.XUGGLER_BGR_24, newPic);
          if (mScreen != null)
            mScreen.setImage(imageConverter.toImage(newPic));
          // clock.tick();
        }
         ...
         * will probably allocate more space than just the 1024 (it's not
         * important why).
         */
        IAudioSamples samples = IAudioSamples.make(1024, audioCoder.getChannels());

        /*
         * A packet can actually contain multiple sets of samples (or frames of
         * samples in audio-decoding speak). So, we may need to call decode
         * audio multiple times at different offsets in the packet's data. We
         * capture that here.
         */
        int offset = 0;
        ...
        while (offset < packet.getSize()) {
          ...
          offset += bytesDecoded;
          /*
           * Some decoder will consume data in a packet, but will not be able to
           * construct a full set of samples yet. Therefore you should always
           * check if you got a complete set of samples from the decoder
           */
          if (samples.isComplete()) {
            // note: this call will block if Java's sound buffers fill up, and
            // we're
        ...
        /*
         * This packet isn't part of our video stream, so we just silently drop
         * it.
         */
        do {
        } while (false);
  ...
  private long millisecondsUntilTimeToDisplay(IVideoPicture picture) {
     ...
     * instead you used the packet timestamps, they can be in different units
     * depending on your IContainer, and IStream and things can get hairy
     * quickly.
     */
    ...
    if (mFirstVideoTimestampInStream == Global.NO_PTS) {
      ...
      final long millisecondsTolerance = 50; // and we give ourselfs 50 ms of
                                             // tolerance
  ...
  public void setImageLister(ImageListener imageListener) {
    this.mScreen = imageListener;
  }
}
